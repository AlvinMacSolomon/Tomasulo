...
public class ReorderBuffer {
    ...
    public boolean retireInst() {
        ...
        if (retiree.getOpcode() == INST_TYPE.JAL || retiree.getOpcode() == INST_TYPE.JALR) {
        ...
        } else if (retiree.getOpcode() == INST_TYPE.NOP) {}
          ...
          else if (!retiree.isBranch() && frontQ == regs.robSlot[retiree.getWriteReg()]) {
    ...
    public void readCDB(CDB cdb) {
        ...
        if (t != -1 && cdb.getDataValid() && buff[t] != null) {
            ...
            for (ROBEntry e : buff) {
                if (e != null && e.isStore()) {
                    if      (t == e.storeAddrTag) e.setStrAddr(cdb.getDataValue());
                    else if (t == e.storeDataTag) e.setStrData(cdb.getDataValue());
    ...
    public void updateInstForIssue(IssuedInst inst) {
        ...
        // all src regs will either be assigned a tag, read from reg, or forwarded from
        // ROB
